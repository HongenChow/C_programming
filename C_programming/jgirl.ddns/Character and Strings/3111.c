...
static int count_keyword[453] = {0};
static char new_keyword[453][1024];
...
"天長地久有時盡此恨綿綿無絕期";
    ...
    while (count < uniq) {
        char *start = string;
        char *find = strstr(start, new_keyword[count]);
        int number_find = 0;
        while (find != NULL) {
            number_find++;
            start = (find + strlen(new_keyword[count]));
            find = strstr(start, new_keyword[count]);
        }
        count_keyword[count++] = number_find;
    }
    ...
    for (int i = 0; i < count; i++) 
        if (strlen(new_keyword[idx[i]]) >= N1 * 3 && strlen(new_keyword[idx[i]]) <= N2 * 3 && count_keyword[idx[i]] >= M1 && count_keyword[idx[i]] <= M2)
            printf("%s %d\n", new_keyword[idx[i]], count_keyword[idx[i]]);
...
int compare (const void *a, const void *b) {
    ...
    if (strlen(new_keyword[i]) != strlen(new_keyword[j]))
        return (strlen(new_keyword[i]) - strlen(new_keyword[j]));
 
    if (count_keyword[i] != count_keyword[j])
        return count_keyword[j] - count_keyword[i];
 
    return strcmp(new_keyword[i], new_keyword[j]);
}